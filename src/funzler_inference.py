#!usr/bin/Python3
"""Class for the main inference algorithm of the diagnosis concept"""
...
class FunzlerInference(object):
    ...
    def run(self):
        """
        Run the inference: search for single boundary solution and double boundary solution, if needed.
        """
        ...
        if np.amax(indices_single["plausibility"]) < self.params["thres_find_double_bouc"]:
            self.csa_2combi, self.bouc_2combi = prepare_2combi(
                self.csa * self.int_1_single)
    ...
    def _get_intensity_matrix(self, inf_type="single"):
        ...
        :param inf_type: inference type, "single" or "double"
        """
    ...
    def compute_indices(self, obs, inf_type="single"):
        ...
        :param inf_type: inference type
        :return indices: dictionary of output indices
        """
        ...
        indices = {"plausibility": plausi, "consistency": cons,
                   "relevance": rel, "cover": cov}
